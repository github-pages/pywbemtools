...
from ._common import display_cim_objects, parse_wbemuri_str, \
    ...
    process_invokemethod, AssociationShrub
...
def instance_shrub(context, instancename, **options):
    ...
    of all instances that are part of the shtub if one of the cim object
...
def get_instancename(context, instancename, options):
    ...
    try:
        if options['interactive']:
            ...
            try:
                return pick_instance(context, instancename, namespace=ns)
            except ValueError:
...
def cmd_instance_shrub(context, instancename, options):
    ...
    not_implemented = ["assocclass", "resultclass", "role", "resultrole"]
    for option in options:
        if option in not_implemented and options[option] is not None:
            raise click.ClickException("%s not implemented" % option)

    shrub = AssociationShrub(context, instancepath)

    click.echo(shrub.build_ascii_display_tree())
