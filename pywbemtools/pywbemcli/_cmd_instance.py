...
@add_options(namespace_option)
@add_options(summary_option)
@click.option('--fullpath/--no-fullpath', default=False,
              help='If False, the path displayed for the tree view is '
                   'compressed to hide path components (replaced with ~ that '
                   'are duplicated for multiple instances. In addition, '
                   'a key named "CreationClassName" keyis also hidden. ')
@click.pass_obj
